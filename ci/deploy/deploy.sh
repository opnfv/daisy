...
{
    bridge_name=$(grep "<source * bridge" $BMDEPLOY_DAISY_SERVER_VM | awk -F "'" '{print $2}')
    if [ $? -eq 0 ] && [ ${bridge_name} != ${BRIDGE} ] && [ ! -z ${bridge_name} ]; then
        line=$(awk "(/<source * bridge/){print NR}" $BMDEPLOY_DAISY_SERVER_VM)
        if [ $? -eq 0 ] && [ ! -z $line ]; then
            echo "Use $BRIDGE to replace the bridge in $BMDEPLOY_DAISY_SERVER_VM"
            sed -i "${line} c\      <source bridge=\'${BRIDGE}\'/>" $BMDEPLOY_DAISY_SERVER_VM
        fi
    fi
}
