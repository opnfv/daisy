...
def foo():
    try:
        ...
        host_interface_map, host_role_map, network_map, vip = get_conf.config(conf['dha'], conf['network'])
        print("add cluster...")
        ...
        add_hosts_interface(cluster_id, hosts_info, host_interface_map, host_role_map, vip)
    except Exception:
...
def update_network(cluster_id, network_map):
    ...
    for net in network_info_list:
        ...
        if network_map.get(network_name):
            ...
            network_info = client.networks.update(network_id,**network_meta)
...
def add_hosts_interface(cluster_id, hosts_info, host_interface_map, host_role_map, vip):
    hosts_id_list = []
    for host in hosts_info:
        ...
        for interface in host['interfaces']:
            ...
            interface['assigned_networks']=host_interface_map[host_name][interface_name]
        client.hosts.update(host['id'], **host)
        print("update role...")
        add_host_role(cluster_id, host['id'], host['name'], host_role_map, vip)
...
def add_host_role(cluster_id, host_id, host_name, host_role_map, vip):
    ...
    lb_role_id  = [role.id for role in role_list if role.name == "CONTROLLER_LB"][0]
    computer_role_id  = [role.id for role in role_list if role.name == "COMPUTER"][0]
    if "CONTROLLER_LB" in host_role_map[host_name]:
        role_lb_update_meta ={'nodes': [host_id], 'cluster_id': cluster_id, 'vip': vip}
        client.roles.update(lb_role_id, **role_lb_update_meta)
    if "COMPUTER" in host_role_map[host_name]:
        role_computer_update_meta ={'nodes': [host_id], 'cluster_id': cluster_id}
