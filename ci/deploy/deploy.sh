...
# BEGIN of main
#
while getopts "b:B:Dn:L:l:p:r:w:s:dr:Sh" OPTION
do
    case $OPTION in
        b)
            BASE_PATH=${OPTARG}
...
RUN_DOCTOR=$(RUN_DOCTOR:-"run_doctor")
...
function config_daisy()
{
    ...
    ssh $SSH_PARAS $DAISY_IP "bash $REMOTE_SPACE/deploy/prepare.sh -n $NETWORK -b $IS_BARE -dr $RUN_DOCTOR"
}
