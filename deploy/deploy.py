...
class DaisyDeployment(object):
    def __init__(self, lab_name, pod_name, deploy_file, net_file, bin_file,
        ...
        with open(deploy_file) as yaml_file:
            self.deploy_struct = yaml.load(yaml_file)

        if not cleanup_only:
            self.net_file = net_file
            with open(net_file) as yaml_file:
                self.net_struct = yaml.load(yaml_file)
        else:
    ...
    def get_daisy_server_info(self):
        ...
        password = self.deploy_struct['daisy_passwd']
        disk_size = self.deploy_struct['disks']['daisy']
    ...
    def run(self):
        ...
        server = DaisyServer(self.daisy_server_info['name'],
                             ...
                             self.adapter)
        server.connect()
        server.install_daisy()
        if self.daisy_only:
            return

        server.prepare_cluster(self.deploy_file, self.net_file)
        self.daisy_env.create_or_find_nodes()
        server.prepare_host_and_pxe()
        if self.adapter == 'libvirt':
            LI('Begin Daisy virtual-deploy os and openstack')
            self.daisy_env.reboot_virtual_nodes()
            LI('Sleep 20s to wait the VM(s) startup')
            time.sleep(20)
            server.install_virtual_nodes()

        LI('The hosts number is %d' % len(self.deploy_struct['hosts']))
        server.check_os_installation(len(self.deploy_struct['hosts']))
        time.sleep(10)
        if self.adapter == 'libvirt':
            self.daisy_env.reboot_virtual_nodes(boot_devs=['hd'])
        server.check_openstack_installation(len(self.deploy_struct['hosts']))
        server.post_deploy()
