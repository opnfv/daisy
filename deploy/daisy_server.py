...
def log_scp(filename, size, send):
    ...
    prefix = 'SCP: '
    size_str = " " * (6 - len(str(size))) + str(size)

    if len(filename) <= MAX_LEN_OF_NAME:
        name_str = filename + ' ' + '.' * (MAX_LEN_OF_NAME - len(filename))
        LD('SCP: ' + name_str + size_str + unit + ' 100%')
    else:
        LD('SCP: ' + filename)
        size_str = ' ' * (MAX_LEN_OF_NAME + 1 + len(prefix)) + size_str
        LD(size_str + unit + ' 100%')
...
class DaisyServer(object):
    ...
    def prepare_files(self):
        ...
        update_config(join(WORKSPACE, 'deploy/daisy.conf'),
                      'daisy_management_ip',
                      self.address,
                      section='DEFAULT')
        LI('Copy daisy.conf to Daisy Server')
        self.scp_put(join(WORKSPACE, 'deploy/daisy.conf'), '/home/daisy_install/')

        if os.path.dirname(os.path.abspath(self.bin_file)) != WORKSPACE:
            LI('Copy opnfv.bin to Daisy Server')
            self.scp_put(self.bin_file, join(self.remote_dir, 'opnfv.bin'))

    def install_daisy(self):
        self.prepare_files()
        LI('Begin to install Daisy')
        status = self.ssh_run('%s install' % join(self.remote_dir, 'opnfv.bin'))
    ...
    def prepare_configurations(self):
        ...
        cmd = 'bash {script} -n {net_file}'.format(
            script=join(self.remote_dir, 'deploy/prepare.sh'),
            net_file=join(self.remote_dir, 'network.yml'))
        self.ssh_run(cmd)

    def prepare_cluster(self, deploy_file, net_file):
        LI('Copy cluster configuration files to Daisy Server')
        self.scp_put(deploy_file, join(self.remote_dir, 'deploy.yml'))
        self.scp_put(net_file, join(self.remote_dir, 'network.yml'))
        ...
        cmd = "python {script} --dha {deploy_file} --network {net_file} --cluster \'yes\'".format(
            script=join(self.remote_dir, 'deploy/tempest.py'),
            deploy_file=join(self.remote_dir, 'deploy.yml'),
            net_file=join(self.remote_dir, 'network.yml'))
    ...
    def check_os_installation(self, nodes_num):
        ...
        cmd = '{script} -d {is_bare} -n {nodes_num}'.format(
            script=join(self.remote_dir, 'deploy/check_os_progress.sh'),
            is_bare=1 if self.adapter == 'ipmi' else 0,
            nodes_num=nodes_num)
        self.ssh_run(cmd, check=True)
    ...
    def check_openstack_installation(self, nodes_num):
        ...
        cmd = '{script} -n {nodes_num}'.format(
            script=join(self.remote_dir, 'deploy/check_openstack_progress.sh'),
            nodes_num=nodes_num)
        self.ssh_run(cmd, check=True)
    ...
    def post_deploy(self):
        ...
        LI('Post deploy ...')
        cmd = 'bash {script} -n {net_file}'.format(
            script=join(self.remote_dir, 'deploy/post.sh'),
            net_file=join(self.remote_dir, 'network.yml'))
