...
def clean_deploy_host(host_ip_passwd_map):
    for host_ip_passwd in host_ip_passwd_map:
        command = 'sshpass -p %s ssh %s -o UserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no "/home/daisy/forDel/tools/cleanup-containers"' % (host_ip_passwd['passwd'], host_ip_passwd['ip'])
        subprocess.call(command,
                        shell=True,
                        stdout=open('/dev/null', 'w'),
                        stderr=subprocess.STDOUT)
        command = 'sshpass -p %s ssh %s -o UserKnownHostsFile=/dev/null -oStrictHostKeyChecking=no "/home/daisy/forDel/tools/cleanup-images"' % (host_ip_passwd['passwd'], host_ip_passwd['ip'])
        subprocess.call(command,
                        shell=True,
                        stdout=open('/dev/null', 'w'),
                        stderr=subprocess.STDOUT)
